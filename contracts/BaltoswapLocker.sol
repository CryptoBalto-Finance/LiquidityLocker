...
contract BaltoswapLocker is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;

  IBaltoswapFactory public baltoswapFactory;
  ...
  struct TokenLock {
    ...
    uint256 lockID; // lockID nonce per BALTO pair
    address owner;
  }
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map baltoswap pair to all its locks
  
  struct FeeStruct {
    uint256 ethFee; // Small eth fee to prevent spam on the platform
    IERCBurn secondaryFeeToken; // BALTO or BORIS
    uint256 secondaryTokenFee; // optional, BALTO or BORIS
    uint256 secondaryTokenDiscount; // discount on liquidity fee for burning secondaryToken
    uint256 liquidityFee; // fee on baltoswap liquidity tokens
  ...
  constructor(IBaltoswapFactory _baltoswapFactory) public {
    devaddr = msg.sender;
    ...
    baltoswapFactory = _baltoswapFactory;
  }
  ...
  /**
   * @notice set the migrator contract which allows locked lp tokens to be migrated to baltoswap next version
   */
   ...
   * @param _lpToken the baltoswap token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_eth fees can be paid in eth or in a secondary token such as BALTO with a discount on baltoswap tokens
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    // ensure this pair is a baltoswap pair by querying the factory
    IBaltoswapPair lpair = IBaltoswapPair(address(_lpToken));
    address factoryPairAddress = baltoswapFactory.getPair(lpair.token0(), lpair.token1());
    require(factoryPairAddress == address(_lpToken), 'NOT BALTO');
    ...
    // record the lock for the baltoswappair
  ...
  function relock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _unlock_date) external nonReentrant {
    ...
    // send baltoswap fee to dev address
    TransferHelper.safeTransfer(_lpToken, devaddr, liquidityFee);
  }
  ...
  /**
   * @notice migrates liquidity to baltoswap next version
   */
